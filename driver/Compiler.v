...
Require LustreVGen.
Require LustreSGen.
Require SimplConstblock.
Require Toposort.
Require SimplLustreS.
Require LustreRGen.
...
Require SimplConstblockProof.
Require ToposortNodesProof.
Require ToposortStmtsProof.
Require Lu2ltProof.
Require Lt2lsProof.
Require SimplLustreSProof.
...
Theorem transf_lts_program_correct:
  ...
  Lglobalenv.initial_state prog1 gc LsemT.init_node main1 e->
...
Proof.
  ...
  destruct (SimplConstblock.trans_program prog1) eqn:?; simpl in *; try congruence.  
  destruct (Toposort.toposort_nodes_program p) eqn:?; simpl in *; try congruence.
  eapply SimplConstblockProof.trans_program_correct with (prog2:=p) in H4; eauto.
  destruct H4 as [A A1].
  eapply ToposortNodesProof.trans_program_correct in A1; eauto.
  destruct A1 as [A1 A2].
  apply SimplConstblock.trans_program_ids_range with (p':=p) in H0; auto.
  apply Toposort.toposort_nodes_program_ids_range with (p':=p0) in H0; auto.  

  eapply ToposortStmtsProof.trans_program_correct in A2; eauto.
  destruct A2 as [main2 [A2 [A3 [A4 A5]]]].
  eapply Toposort.toposort_stmts_program_ids_range in H0; eauto.

  eapply Lu2ltProof.exec_prog_correct in A3; eauto.
  
  exists main2. repeat (split; auto).
  +apply ToposortStmtsProof.toposort_stmts_nodes_topo_sorted with p0; auto.
  ...
  +eapply Toposort.toposort_stmts_program_gids_range; eauto.
   ...
   eapply SimplConstblock.trans_program_gids_range; eauto.
  +rewrite ToposortStmtsProof.toposort_stmts_program_globids with p0 prog2; auto. 
   eapply ToposortNodesProof.toposort_nodes_program_globids_norepet; eauto. 
   eapply SimplConstblockProof.trans_program_globids_norepet; eauto. 
...
Theorem transf_lsr_program_correct:
  ...
  exists main2 gc2, Lenv.initial_state1 prog2 gc2 LsemR.init_node main2 e
    /\ Lenv.exec_prog1 prog2 gc2 (LsemR.eval_node false) main2 e 1 maxn mass vrss
    /\ LustreR.ids_range ACG_J (node_block prog2)
...
Proof.
  ...
  apply Lt2lsProof.exec_prog_correct in H5; auto.
  remember (SimplLustreS.trans_program prog1) as t1.
  destruct t1; simpl in H; try congruence.
  eapply SimplLustreSProof.trans_program_correct with (prog2:=p) in H5; eauto.
  destruct H5 as [main2 [gc2 [? [? [? A]]]]].

  eapply SimplLustreS.trans_program_ids_range in H1; eauto.
  
  remember (LustreRGen.trans_program p) as t2. 
  destruct t2; simpl in H; try congruence.
  apply LustreRGenProof.trans_program_correct with (progR:=p0) in A; auto.
  destruct A as [main3 [? [? [? A]]]].
  
  apply Toposort.topo_sorted_nodes_simpl in H3.
  erewrite SimplLustreS.trans_program_deps_of_nodes_simpl in H3; eauto.

  rewrite LustreRGen.trans_program_deps_of_nodes with (p':=p0) in H3; auto.
  eapply SimplLustreS.trans_program_ids_plt in H2; eauto.
  erewrite <-LustreRGen.trans_program_globidspltof in H2; eauto.
  
  destruct (TransMainArgs.trans_program p0) eqn:?; simpl in *; try congruence.
  ...
  exists main5, gc2. inv H. repeat (split; auto); try congruence.
  +apply TransTypecmp.trans_program_ids_range with p1; auto.
   eapply TransMainArgs.trans_program_ids_range; eauto.
   apply LustreRGen.trans_program_ids_range with p; eauto.
  +eapply TransTypecmpProof.trans_program_ids_plt; eauto.
  +eapply TransTypecmp.trans_program_gids_norepet; eauto.
   eapply TransMainArgs.trans_program_gids_norepet; eauto.
   rewrite LustreRGen.trans_program_gids_eq with (p:=p); auto.
   eapply SimplLustreS.trans_program_gids_norepet; eauto.
  +eapply TransMainArgs.trans_program_gids_norepet with (p:=p0); eauto.
   rewrite LustreRGen.trans_program_gids_eq with (p:=p); auto.
   eapply SimplLustreS.trans_program_gids_norepet with (p:=prog1); eauto.
  +eapply LustreRGen.trans_program_ids_range with (p:=p); eauto.
  +congruence.
Qed.
...
Theorem transf_lrd_program_node_correct:
  ...
  LustreR.ids_range ACG_J (Lustre.node_block prog1) ->
...
Proof.
  ...
  exists mrss, main5, e3. split; [| split; [| split; [| split]]]; auto.
  ...
  -eapply ResetfuncGen.trans_program_ids_range; eauto.
   ...
   apply LustreR.ids_range_le with ACG_J; auto.
   unfold Ple, OUTSTRUCT, ACG_J. omega.
  -unfold ResetfuncGen.trans_program in Heqr0.
   destruct (list_in_list _ _); inv Heqr0.
   destruct (check_norepeat _); inv H18. auto.
Qed. 
...
Theorem transf_lt_program_correct:
  ...
  Lglobalenv.initial_state prog gc LsemT.init_node main e->
