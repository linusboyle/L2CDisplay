...
Fixpoint loadbytes_store_init_data (m: mem) (b: block) (p: Z) (il: list init_data) {struct il} : Prop :=
  ...
  | Init_addrof symb ofs :: il' =>
      loadbytes_store_init_data m b (p + 4) il'
  | Init_space n :: il' =>
      Mem.loadbytes m b p (Zmax n 0) = Some (list_repeat (nat_of_Z (Zmax n 0)) (Byte Byte.zero))
...
Lemma store_init_data_outside_bytes:
  ...
  forall n b' q,
  b' <> b \/ q + n <= p \/ p + Genv.init_data_size a <= q->
...
Proof.
  ...
  eapply store_init_data_outside_bytes; eauto.
  destruct C; auto.
Qed.
...
Lemma store_init_data_list_bytes:
  forall b il m p m',
  Genv.store_init_data_list ge m b p il = Some m' ->
  Mem.loadbytes m b p (Genv.init_data_list_size il) = Some (list_repeat (nat_of_Z (Genv.init_data_list_size il)) (Byte Byte.zero)) ->
...
Proof.
  ...
  intros until m'. caseEq (Genv.store_init_data ge m b p a); try congruence. 
  intros m1 B C C1.
  generalize (Genv.init_data_list_size_pos il) (Genv.init_data_size_pos a).
  intros. apply Mem.loadbytes_split in C1; try omega.
  destruct C1 as [? [? [C1 [C2 C3]]]].
  rewrite Z2Nat.inj_add in C3; try omega. rewrite list_repeat_app in C3. 
  apply app_length_equal_inv in C3.
  destruct C3. subst.

  exploit IHil; eauto.
  
  unfold nat_of_Z. rewrite <-C2.
  eapply store_init_data_outside_bytes; eauto.  
  right. omega.
  ...
  simpl in *. 
  unfold nat_of_Z. rewrite <-C1.
  eapply store_init_data_list_outside_bytes; eauto.  
  right. omega.

  left. rewrite length_list_repeat.
  apply Mem.loadbytes_length in C1. auto.
Qed.
...
Proof.
  ...
  +inv H1. exists (Mem.nextblock m); repeat split. 
    ...
    *generalize (Genv.init_data_list_size_pos init). intros.
     eapply store_init_data_list_bytes; eauto.
     simpl in *. rewrite <-nat_of_Z_eq with (z:=Genv.init_data_list_size init) at 1; auto.
     eapply Genv.store_zeros_loadbytes; eauto; try omega.
