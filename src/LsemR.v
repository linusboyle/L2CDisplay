...
Inductive eval_node: env -> env -> ident*node -> list val -> list val -> Prop :=
  | exec_node: forall te te1 te2 eh eh1 eh3 se se' vrs vas nid f,
      ...
      eval_fbyeqs gc te2 eh1 eh3 f.(nd_eqt) ->
      eqt_counter f.(nd_eqt) = fbyn_counter f.(nd_stmt) ->
      incl (flat_map lidsof_eqt f.(nd_eqt)) (map fst (nd_flags f ++ nd_svars f)) -> 
      incl (fbyvarsof f.(nd_stmt)) (nd_svars f) ->
...
with eval_stmt: bool -> locenv -> env -> locenv -> env -> stmt  -> Prop :=
  ...
  | eval_Sfby_cycle_1: forall te te1 eh se lh id flag a1 a2,
      eval_sexp empty_locenv eh (Svar flag type_bool) Vtrue ->
      eval_eqf gc te te1 (lh,a2) ->
      eval_stmt true te (mkenv eh se) te1 (mkenv eh se) (Sfby lh id flag a1 a2)
  | eval_Sfby_cycle_n: forall te te1 eh se lh id flag a1 a2 v1 v,
      eval_sexp empty_locenv eh (Svar flag type_bool) Vfalse ->
      ...
      ~ In id (get_lids lh) ->
      eval_stmt true te (mkenv eh se) te1 (mkenv eh se) (Sfby lh id flag a1 a2)
  | eval_Sfbyn_cycle_1: forall te te1 eh eh1 eh2 se lh id1 id2 aid flag sa aty i a1 a2 v1 v2 v,
      ...
      eval_stmt true te (mkenv eh se) te1 (mkenv eh2 se) (Sfbyn lh (id1,id2,aid) flag i a1 a2)
  | eval_Sfbyn_cycle_n: forall te te1 eh se lh id1 id2 aid flag sa aty i a1 a2 v1 v,
      Tarray aid (typeof a1) (Int.unsigned i) = aty ->
      Svar id1 (make_fbyn_type id2 aty) = sa ->
      eval_sexp empty_locenv eh (Svar flag type_bool) Vfalse->
      ...
      locenv_setlvar gc te lh v te1 ->
      ~ In id1 (get_lids lh) ->
      eval_stmt true te (mkenv eh se) te1 (mkenv eh se) (Sfbyn lh (id1,id2,aid) flag i a1 a2)
  | eval_Sarrow: forall te te1 eh se lh flag a1 a2 v b,
      eval_sexp empty_locenv eh (Svar flag type_bool) v ->
      bool_val v type_bool = Some b ->
      eval_eqf gc te te1 (if b then (lh,a1) else (lh,a2)) ->
      eval_stmt true te (mkenv eh se) te1 (mkenv eh se) (Sarrow lh flag a1 a2)
...
Proof.
 ...
 +simpl in *. destruct IHeval_node as [l [vl [? ?]]].
   ...
   *apply ids_norepet_vars_args_rets_disjoint in H1; auto.
    ...
    apply in_map with (f:=fst) in H11; auto.
...
Proof.
  ...
  +apply list_disjoint_app_left in H3. destruct H3.
...
Proof.
  ...
  +constructor 1; auto.
   ...
   eapply get_stmt_nids_incl in H2; eauto.
   unfold incl in *. auto. 
  +constructor.
...
Definition mkprog(p: program)(l1: list (ident*node))(tys: list (ident * type)):=
  mkprogram (p.(type_block) ++ tys) p.(const_block) l1 p.(node_main).

Definition mkprog1(p: program)(l1 l2: list (ident*node))(main: ident*node)(tys: list (ident * type)):=
  mkprogram (p.(type_block) ++ tys) p.(const_block) (l1 ++ main :: l2) p.(node_main).
