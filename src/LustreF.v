...
Definition call_func(ge: list (ident*func))(cdef: calldef)(fd: ident*func) : Prop :=
...
Lemma ids_norepet_rets_svars:
  forall f, ids_norepet f ->
  list_norepet (map fst (svarsof f)).
Proof.
  intros. destruct H as [? [? [? ?]]].
  unfold allidsof, allvarsof, predidof, svarsof in *.
...
Lemma ids_norepet_rets_svars_disjoint:
  forall f, ids_norepet f ->
  list_disjoint (map fst (nd_rets f)) (map fst (nd_flags f ++ nd_svars f)).
Proof.
  intros. apply ids_norepet_rets_svars in H.
  unfold svarsof in *. rewrite map_app in H.
...
Proof.
  intros. apply ids_norepet_rets_svars in H.
  unfold svarsof in *. rewrite map_app in *.
  apply list_norepet_app in H. intuition. 
Qed.
