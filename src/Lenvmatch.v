...
Lemma ptree_ids_match_permut:
  forall l1 l2 e1 e2,
  ptree_ids_match l1 e1 e2 ->
  Permutation l1 l2 ->
  ptree_ids_match l2 e1 e2.
Proof.
  intros. red; intros.
  apply H. apply Permutation_sym in H0.
...
Lemma eval_clock_conds_ptree_ids_match:
  forall gc e1 l b,
  eval_clock_conds gc e1 l b ->
  forall e2, ptree_ids_match (flat_map get_expids l) e1 e2 ->
  eval_clock_conds gc e2 l b.
Proof.
  induction 1; simpl; intros.
  +constructor.
  ...
  +apply ptree_ids_match_app in H3. destruct H3.
...
Lemma ptree_ids_match_eval_fbyn_init:
  ...
  forall eh2 cks, ptree_ids_match (flagid cks :: id1 :: nil) eh1 eh2 ->
  exists eh2' l ml, eval_fbyn_init gc id1 id2 aid t i j v2 eh2 eh2'
    /\ ptree_ids_match (flagid cks :: id1 :: nil) eh1' eh2'
    /\ incl l (id1 :: nil)
...
Proof.
  ...
  +subst. eapply ptree_ids_match_locenv_setlvar_exists in H3; simpl; eauto.
   ...
   destruct IHeval_fbyn_init with eh21 cks as [eh2' [l [ml [? [? [? [? ?]]]]]]]; auto.
...
Lemma alloc_variables_disjoint_locenv_range_perm_vars:
  forall e l1 e',
  alloc_variables e l1 e' ->
  forall l2, list_disjoint (map fst l2) (map fst l1) ->
  locenv_range_perm_vars e l2 ->
  locenv_range_perm_vars e' l2.
Proof.
  induction 1; simpl; intros; auto.
  apply IHalloc_variables; auto.
  red; intros. eapply H0; simpl; eauto.
  red. intros. red. rewrite PTree.gso; auto.
  apply H1; auto. eapply H0; simpl; eauto.
  apply in_map with (f:=fst) in H2; auto.
Qed.
...
Lemma locenv_setvars_ptree_ids_none:
  ...
  forall ids, ptree_ids_none ids e ->
  ptree_ids_none ids e'.
Proof.
  induction 1; intros; auto.
  apply IHlocenv_setvars. 
  inv H0. red; intros.
  rewrite H3 in H. inv H.
  apply H2 in H0. compare id id0; intros.
  subst. congruence.
  rewrite PTree.gso; auto.
Qed.

Lemma locenv_setlvars_ptree_ids_none:
  forall gc e al vl e',
  locenv_setlvars gc e al vl e' ->
  forall ids, ptree_ids_none ids e ->
  ptree_ids_none ids e'.
Proof.
  induction 1; intros; auto.
  apply IHlocenv_setlvars. 
  eapply locenv_setlvar_ptree_ids_none; eauto.
Qed.
