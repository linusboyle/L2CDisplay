...
Definition trans_const(c: const)(ty: type): expr :=
  ...
  | Cenum id idl => Econst_int (Int.repr (count_occ_pos idl id 0)) ty
  end.
...
Fixpoint trans_cases(lh: expr)(pel: list (patn*sexp)): labeled_statements :=
  ...
  | (p, e) :: petl =>
    ...
    | Paenum id idl => LScase (Int.repr (count_occ_pos idl id 0)) cs rs
    end
  end.
...
Fixpoint trans_stmt(s: stmt): statement :=
  ...
  | LustreF.Scall oid lh cdef vals =>
     ...
     let cty := Tfunction (trans_ptypes cdef) Tvoid in
...
Definition trans_body(b: func): fundef :=
  ...
  Internal (mkfunction Tvoid (trans_paras b) (nd_vars b) (trans_rets b) s).
...
Lemma trans_fundef_type:
  ...
  rettys c = nd_rets f ->
  type_of_fundef (trans_body f) = Tfunction (trans_ptypes c) Tvoid.
Proof.
