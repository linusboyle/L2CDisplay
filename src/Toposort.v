...
Definition toposort_stmts (eqs: list (stmt*clock)) : res (list (stmt*clock)):=
  let depl :=  deps_of_stmts eqs in 
  do depl' <- toposort_deps (length depl) depl; 
  let ids := map seqn depl' in
  OK (map (fun id => nth id eqs (Sskip,nil)) ids).

Definition toposort_node (b: node) : res node :=
  do eqs' <- toposort_stmts b.(nd_stmt);
  OK (mknode b.(nd_kind) b.(nd_args) b.(nd_rets) nil nil b.(nd_vars) eqs' b.(nd_sid) b.(nd_fld) nil).

Definition get_hstmt_nid  (h: hstmt): list ident :=
  match h with
  | Hmapcall _ c _ => callid c :: nil
  | Hfillcall _ _ _ c _ => callid c :: nil
  | Hredcall _ _ c _ _ => callid c :: nil
  | Hmapfoldcall _ _ _ c _ _ => callid c :: nil
  | _ => nil
  end.

Definition get_stmt_nid  (eq: stmt* clock): list ident :=
  match fst eq with
...
Definition toposort_nodes_program (p: program): res program :=
  ...
  OK (mkprogram p.(type_block) p.(const_block) nodes p.(node_main)).
...
Lemma deps_of_stmts_map: forall (eqs: list (stmt*clock)),
...
Proof.
  ...
  apply Permutation_sym in EQ0. apply Permutation_app.
...
Definition is_eqs_toposorted (l1 l2:list (stmt*clock)) : Prop :=
...
Lemma eqs_map_seq_perm: forall (a: depend)(l: list (stmt*clock))(depl: list depend),
  Permutation (deps_of_stmts l) depl ->
  In a depl ->
  lidl a = lidl_of_s (fst (nth (seqn a) l (Sskip,nil))) /\
  ridl a = ridl_of_sc (nth (seqn a) l (Sskip,nil)).
Proof.
