...
Definition trans_calldef(ntl: list (ident*func))(c: calldef): res calldef :=
  ...
  let sid := acg_outc_name (fst fd) in 
...
Definition trans_body (f: ident*func)(ntl: list (ident*func)): res func :=
  ...
  OK (mknode b.(nd_kind) b.(nd_args) b.(nd_rets) b.(nd_flags) b.(nd_svars) b.(nd_vars) 
           s1 (acg_outc_name (fst f)) b.(nd_fld) nil).
...
Fixpoint calls_of(s: stmt)(ntl: list (ident*func)): res (list (ident*type)) :=
  ...
  | Scall oid _ (mkcalldef kd isid cnid oj _ _ _ _ ) _ =>
     if kd then   
        ...
        let sty := Tstruct (acg_outc_name (fst fd)) (nd_fld (snd fd)) in
        match oj with
...
Definition get_nodetype(fd: ident*func)(ntl: list (ident*func)): res (ident*func):=
  ...
  OK (fst fd, mknode f.(nd_kind) f.(nd_args) f.(nd_rets) f.(nd_flags) f.(nd_svars) 
                f.(nd_vars) f.(nd_stmt) f.(nd_sid) (fieldlist_of decls) nil).
...
Definition trans_nodetype(fd: ident*func): list (ident*type) :=
  if (nd_kind (snd fd)) then
    ...
    | Fnil => nil
    | _ => (acg_outc_name (fst fd), Tstruct (acg_outc_name (fst fd)) (nd_fld (snd fd))) :: nil
    end
  else nil.
...
Definition trans_program (p: program): res program :=
  ...
  if list_in_list (map fst sblock) (globidsof p) then
    ...
    if check_norepeat (map fst sblock) then
      OK (mkprogram (type_block p++sblock) (const_block p) fds (node_main p))
    else Error (msg "OutstructGen: out_struct_type names are repeated!").
...
Proof.
  ...
  +destruct c. monadInv H1. destruct cakind; simpl in *.
...
Proof.
  ...
  +destruct c. destruct cakind; simpl in *.
   ...
   inv H1. auto. 
   monadInv H0. simpl in *. monadInv EQ. auto.
  +monadInv H1. simpl. eapply IHs; eauto.
...
Proof.
  ...
  destruct (check_norepeat _); inv H1.
...
Proof.
  ...
  destruct (check_norepeat _); inv H2; auto.
Qed.
...
Proof.
  ...
  destruct (check_norepeat _) eqn:?; inv H1.
  ...
  apply list_norepet_permut with (map fst (trans_nodetypes x) ++ (map fst (type_block p) 
