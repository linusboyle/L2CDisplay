...
Hypothesis ID_RANGE:
  LustreR.ids_range ACG_J (node_block prog1).
...
Lemma alloc_variables_match:
  ...
  exists ta, alloc_variables empty_locenv (allvarsof (trans_body f)) ta
    /\ locenv_matcht (mkloopid (nd_eqt f)) te ta. 
Proof.
  intros.
  unfold allvarsof in *. simpl.
  destruct mkloopid_idrange with (nd_eqt f) as [ A | A]; 
...
Lemma trans_expr_typeof:
  forall a, typeof (trans_expr a) = typeof a.
Proof.
  induction a; simpl; auto.
Qed.

Lemma trans_expr_get_lids:
  forall a, get_lids (trans_expr a) = get_lids a.
Proof.
  induction a; simpl; auto.
Qed.   

Lemma eval_sexp_trans_expr:
  forall ta e a v,
  Lsem.eval_sexp empty_locenv e a v ->
  eval_sexp gc ta e (trans_expr a) v
with eval_lvalue_trans_expr:
  forall ta e a b ofs k,
  Lsem.eval_lvalue empty_locenv e a b ofs k ->
  match k with 
  | Lid => eval_lvalue gc ta e (trans_expr a) b ofs Sid
  | _ => False
  end.
Proof.
 +induction 1; simpl; intros.
  ...
  -constructor 4 with v1; auto.
   rewrite trans_expr_typeof; auto.
  -apply eval_lvalue_trans_expr with (ta:=ta) in H.
   destruct k; try tauto. inv H0.
   constructor 5 with id ofs Sid; simpl; auto;
   rewrite trans_expr_typeof; auto.
 +induction 1; simpl; intros; auto.
  -constructor 3 with m; auto.
  -rewrite PTree.gempty in *. congruence.
...
Lemma eval_clock_conds_true_check_clock:
  forall p nid te l e e' s,
  eval_clock_conds gc te l true ->
  forall al ta, locenv_matcht al te ta ->
  eval_stmt false p gc nid ta e ta e' s ->
  eval_stmt false p gc nid ta e ta e' (check_clock l s).
Proof.
  induction l; simpl; intros; inv H; auto.
  destruct e.
  apply eval_Sif with v true; auto.
  eapply eval_sexp_match; eauto.
  eapply eval_sexp_sexp; eauto.
  congruence.
  eapply IHl; eauto.
...
Lemma eval_clock_conds_false_check_clock:
  forall p nid te l e s,
  eval_clock_conds gc te l false ->
  forall al ta, locenv_matcht al te ta ->
  eval_stmt false p gc nid ta e ta e (check_clock l s).
Proof.
  induction l; simpl; intros; inv H; auto.
  +destruct e.
   apply eval_Sif with v false; auto.
   eapply eval_sexp_match; eauto.
   eapply eval_sexp_sexp; eauto.
  ...
  +destruct e.
   ...
   eapply eval_sexp_match; eauto.
   eapply eval_sexp_sexp; eauto.
   congruence. eapply IHl; eauto.
Qed.

Lemma eval_fbyeqs_correct:
  forall al te eqs eh eh',
  eval_fbyeqs gc te eh eh' eqs ->
  forall ta se nid, locenv_matcht al te ta ->
  eval_stmt false prog2 gc nid ta (mkenv eh se) ta (mkenv eh' se) (trans_peqs eqs).
Proof.
  ...
  +inv H.
   ...
   -eapply eval_clock_conds_true_check_clock; eauto.
    constructor. inv H2.
    cut (eval_sexp gc ta eh a2 v).
    cut (eval_lvalue gc ta eh (trans_expr a1) b ofs Sid). intros.
    constructor 1 with v v'; auto.
    *rewrite trans_expr_typeof; auto.
    *apply lids_disjoint_assign_disjoint with v b ofs Sid; auto.
     rewrite trans_expr_get_lids; auto.
    *rewrite trans_expr_typeof; auto.
    *constructor 2 with b ofs; auto.
     rewrite trans_expr_typeof; auto.
    *eapply eval_lvalue_trans_expr in H6; eauto.
    *eapply eval_sexp_match; eauto.
     eapply eval_sexp_sexp; eauto.
   -eapply eval_clock_conds_true_check_clock; eauto.
    constructor. inv H. inv H5.
    constructor 1 with v v'; simpl; auto.
    *eapply eval_sexp_trans_expr; eauto.
    *repeat rewrite trans_expr_typeof; auto.
    *inv H6. constructor 1 with chunk; auto.
     rewrite trans_expr_typeof; auto.
     constructor 2; auto.
     rewrite trans_expr_typeof; auto.
     inv H9. eapply eval_lvalue_trans_expr with (ta:= ta) in H6; eauto.
     eapply eval_lvalue_trans_expr with (ta:= ta) in H10; eauto.
     destruct k1; try tauto. destruct k2; try tauto.
     constructor 1 with id1 id2 o1 o2 Sid Sid; auto.
     repeat rewrite trans_expr_typeof; auto.
    *rewrite trans_expr_typeof; auto.
    *constructor 2 with b ofs; auto.
     eapply eval_lvalue_trans_expr with (ta:= ta) in H; eauto.
     rewrite trans_expr_typeof; auto.
   -destruct eq; apply eval_Sseq with ta (mkenv eh se); auto;
     eapply eval_clock_conds_false_check_clock; eauto;
     eapply eval_clock_conds_match; eauto.
Qed.
...
Proof.
  ...
  ( P0 := fun nk te e te' e' s =>
      ...
      (fbyn_counter s <= eqt_counter l)%nat ->
      nk = nd_kind (snd nd) ->
      find_funct (node_block prog1) nid = Some nd ->
 ...
 +(*node*)
  ...
  apply locenv_setvars_exists with (l1:=mkloopid (nd_eqt f)) (eh2:=ta) in H0; auto.
  destruct H0 as [ta1 [A2 A3]].
  eapply trans_body_ids_norepet in H1; eauto.
  unfold trans_body in *.
  destruct IHeval_node with nid (nid,f) ta1 (nd_eqt f) as [ta2 [A5 A6]]; auto.
    omega.
  ...
  -apply find_funct_in2 in H9.
   apply ID_RANGE in H9; auto.
 +(*Sassign*)
...
Proof.
 ...
 +(*init_node*)
  ...
  -rewrite trans_peqs_instidof, <-app_nil_end.
