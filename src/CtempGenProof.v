...
Proof.
  ...
  inv H14.
  +(*Lid*)
   inv H20. destruct H with m0 t as [l [C [C1 [C2 C3]]]]; auto.
    ...
    *eapply eval_sem_cast_correct; eauto.
     ...
     inv H12; auto. destruct H16; congruence.
    *eapply eval_cast_has_type; eauto.
  +(*Sid*)
   inv H20. destruct H with m0 t as [l [o' [C [C1 C2]]]]; auto.
   destruct B5 with id m0 t as [l' [o [C3 [? [? [? [? ?]]]]]]]; auto.
   rewrite C1 in C3. inv C3.
   apply in_map with (f:=(fst (B:=int*type))) in H15.
   rewrite trans_expr_typeof in *.
    ...
    *eapply eval_sem_cast_correct; eauto.
     destruct is_arystr_by_value with v (Lustre.typeof a1) as [chunk ?]; auto.
     inv H12; auto. destruct H21; congruence.
    *eapply eval_cast_has_type; eauto. 
 ...
 +(*eval_Scall *)
  ...
  -apply exec_Scall
      with (to_typelist (map trans_ptype (argtys cdef)))
           ...
           (rettys cdef))) (Vptr loc Int.zero) vinC' voutC (trans_body f); auto.
...
Proof.
  ...
  +simpl in *.
   ...
   rewrite A.
   apply Mem.loadbytes_concat; auto; destruct a; try tauto;
   try (destruct (zle _ _));
   try destruct H; try destruct H0; subst;
   try (erewrite loadbytes_firstn_skipn; eauto; fail); 
   try (apply IHl; auto; try omega; fail).
   simpl. rewrite Z.max_r; auto. simpl.
...
Proof.
  ...
  +destruct (alloc_global gc a) eqn:?; try congruence.
   ...
   -erewrite alloc_globals_notin_eq in H1; eauto.
    ...
    repeat (split; auto).
    *generalize Heqo0 (Genv.init_data_list_size_pos gvar_init).
     intros A A1. apply store_zeros_length in A.
     unfold alloc in A. rewrite length_list_repeat in A.
     assert(A2: Genv.init_data_list_size gvar_init = 0 \/ Genv.init_data_list_size gvar_init > 0).
       omega.
     destruct A2 as [A2 | A2].
     apply init_data_list_size_zero_loadbytes_store; auto.
     eapply store_init_datas_bytes with (n:=Z_of_nat (length m)); eauto.     
     rewrite A. rewrite nat_of_Z_eq; try omega.   
     apply store_zeros_content in Heqo0; eauto.
     rewrite A. rewrite nat_of_Z_eq; auto.
     omega.
...
Proof.
 induction 1; intros.
  ...
  assert (fst fi = acg_reset_name (node_main progL)).
    eapply find_funct_fid; eauto.
  assert (fst main = node_main progL).
    eapply find_funct_fid; eauto.
  destruct fi as [fid fi]. destruct main as [nid f]. simpl in *. subst fid nid.
  destruct globfuncs_match with (acg_reset_name (node_main progL)) fi as [loc1 [A2 A3]]; auto.
  ...
  remember (acg_reset_name (node_main progL), fi)as fd.
  assert(A5: nd_args fi = nil).
    clear TRANS. inv H6. inv H18. inv H11. auto.
