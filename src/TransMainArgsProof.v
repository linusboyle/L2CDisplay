...
Proof.
  ...
  +destruct (field_offset _ _) eqn:?.
   -unfold makevar, trans_v. 
    ...
    destruct H8 as [m1 [? ?]]. 
    apply eval_Sfield with (acg_inc_name (fst fd)) (fieldlist_of (nd_args (snd fd))); auto.
...
Lemma node_ids_instruct_notin:
  ~ In INSTRUCT (map fst (node_block prog1)).
Proof.
...
Lemma eval_typecmp_trans:
  ...
  eval_typecmp gc te1 a1 a2 b ->
  locenv_match fd te1 te2 ->
  sizeof (mkstruct fd) <= Int.max_signed ->
  eval_typecmp gc te2 (trans_expr (makevar fd) a1) (trans_expr (makevar fd) a2) b.
Proof.
  intros until te2.
  ...
  ( P0 := fun a1 a2 num aty i b =>
      locenv_match fd te1 te2 ->
      sizeof (mkstruct fd) <= Int.max_signed ->
      eval_arycmp gc te2 (trans_expr (makevar fd) a1) (trans_expr (makevar fd) a2) num aty i b)
  ( P1 := fun a1 a2 fld ftl b =>
      locenv_match fd te1 te2 ->
      sizeof (mkstruct fd) <= Int.max_signed ->
      eval_strcmp gc te2 (trans_expr (makevar fd) a1) (trans_expr (makevar fd) a2) fld ftl b);
 ...
 +apply eval_typecmp_array with aid aty num v1 v2; auto;
  try (repeat rewrite trans_expr_typeof; auto;
    eapply locenv_match_eval_sexp; eauto).
 +apply eval_typecmp_struct with (sid:=sid) (fld:=fld) (v1:=v1) (v2:=v2); auto;
  try (repeat rewrite trans_expr_typeof; auto;
    eapply locenv_match_eval_sexp; eauto).
Qed.
...
Proof.
  ...
  +(*Scall*)
    ...
    *eapply locenv_match_ptree_id_none; eauto. destruct H1.
     ...
     rewrite <-A. apply in_map. eapply find_funct_in2 in H1; eauto.
  +(*Sfor_start*)
   eapply locenv_match_eval_eqf in H; eauto.
   destruct H as [te21 [? ?]].
  ...
  +(*Stypecmp*)
   ...
   -eapply eval_typecmp_trans; eauto.
   -apply trans_expr_lid_disjoint; auto.
   -rewrite trans_expr_typeof; auto.
...
Lemma trans_get_lids_list_disjoint:
  forall l fd s,
  list_disjoint l (get_lids s) ->
  ~ (In INSTRUCT l) ->
  list_disjoint l (get_lids (trans_expr (makevar fd) s)).
Proof.
  induction s; simpl; intros; auto.
  unfold makevar, trans_v.
  destruct (in_list _ _); simpl; auto. 
  red; simpl; intros. destruct H2; try tauto.
  subst. red; intros. subst. auto.
Qed.

Lemma eval_clock_conds_match:
  forall te1 conds b,
  eval_clock_conds gc te1 conds b ->
  forall fd te2, locenv_match fd te1 te2 ->
  sizeof (mkstruct fd) <= Int.max_signed ->
  eval_clock_conds gc te2 (map (trans_expr (makevar fd)) conds) b.
Proof.
  induction 1; simpl; intros.
  +constructor.
...
Lemma trans_stmt_eval_fbyeqs:
  forall l fd te1 eh eh' te2,
  eval_fbyeqs gc te1 eh eh' l ->
  locenv_match fd te1 te2 ->
  ~(In INSTRUCT (flat_map lidsof_eqt l)) ->
  sizeof (mkstruct fd) <= Int.max_signed ->
  eval_fbyeqs gc te2 eh eh' (map (trans_eqt (makevar fd)) l).
Proof.
  induction l; simpl; intros; eauto. 
  +inv H. constructor.
  +inv H. inv H8.
   -econstructor; eauto.
    *unfold trans_exprs. rewrite <-map_rev.
...
Lemma trans_stmt_fbyn_counter:
  forall mkv s,
  fbyn_counter (trans_stmt mkv s) = fbyn_counter s.
Proof.
  induction s; simpl; intros; auto.
Qed.

Lemma trans_peqs_eqt_counter:
  forall mkv l, eqt_counter (map (trans_eqt mkv) l) = eqt_counter l.
Proof.
...
Lemma trans_peqs_lidsof:
  forall mkv l,
  flat_map lidsof_eqt (map (trans_eqt mkv) l) = flat_map lidsof_eqt l.
Proof.
  induction l; simpl; intros; auto.
  rewrite IHl. destruct a; simpl; auto.
  destruct e; simpl; auto.
  unfold lidsof_eqt. destruct e; auto.
Qed.
...
Proof.
  ...
  apply ids_plt_le_notin with INSTRUCT _ _ in H10; try (unfold Ple; omega).
  ...
  apply not_in_app in H10. destruct H10 as [? A5].
  ...
  destruct H2 as [te2' [? ?]].
  eapply locenv_match_getvars in H7; eauto.
  apply exec_node with te21 te22 te2' eh1; auto.
  +unfold trans_body, makevar in *. simpl in *.
   eapply trans_stmt_eval_fbyeqs in H3; eauto.
   unfold makevar in *; auto.
   unfold mkstruct, sizeof_fld in *. simpl in *. omega.
  +simpl. rewrite trans_stmt_fbyn_counter.
   rewrite trans_peqs_eqt_counter; auto.
  +simpl. rewrite trans_peqs_lidsof. 
   rewrite map_app; auto.
  +simpl. change (trans_v _ _) with (makevar (nid,f)).
   rewrite trans_body_fbyvarsof; auto.
  +simpl. apply list_norepet_app in A.
   destruct A as [? [? A]]. eapply list_disjoint_app_left; eauto.
  +red; intros. apply H10. simpl. in_tac.
  +red; intros. apply A4. in_tac.
  +omega.
  +red; intros. apply A5. simpl.
   apply in_map_iff in H18. destruct H18 as [? [? A7]].
   apply H6 in A7. apply in_map with (f:=fst) in A7.
   apply in_or_app; auto. right. congruence.
  +simpl in *. omega.
