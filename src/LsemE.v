...
Inductive init_node: env -> ident*func -> Prop :=
  | init_node_: forall eh eh1 se nid f,
      ...
      ids_norepet f ->
      eval_init eh (nd_flags f) (nd_svars f) eh1 -> 
      svars_fld_match (svarsof f) (nd_fld f) ->
...
Inductive init_env: env -> env -> ident*func -> Prop :=
  | init_env_: forall eh eh1 se se1 nid f,
      ids_norepet f ->
      locenv_range_perm_vars eh (nd_flags f) ->
      locenv_setvars eh (nd_flags f) (list_repeat (length (nd_flags f)) Vtrue) eh1 ->
      (forall ty, In ty (map snd (nd_flags f)) -> ty = type_bool) ->
      svars_fld_match (svarsof f) (nd_fld f) ->
...
Proof.
  ...
  -constructor 1; simpl; auto.
   apply alloc_variables_app in H4.
   destruct H4 as [e0 [? ?]].
   apply ids_norepet_rets_svars in H0; auto.
   unfold svarsof in H0. rewrite map_app in H0. 
   apply list_norepet_app in H0. destruct H0 as [? [A3 ?]]; auto.
   rewrite map_app in A3. apply list_norepet_app in A3.
   destruct A3 as [? [? ?]].
   eapply alloc_variables_disjoint_locenv_range_perm_vars; eauto.   
   eapply alloc_variables_range_perm; eauto.
   intros. rewrite H5 with ty; simpl; auto.
   unfold Int.max_signed. simpl. omega.
   apply in_map with (f:=snd) in H11; auto.
 +(*nil*)
  subst. inv H0.
...
Proof.
  intros. inv H; inv H1.
  +destruct H0 as [? [? [? ?]]]. destruct H4.
   eapply list_fld_match_nth; eauto.
...
Proof.
  ...
  +inv H4. destruct H3 as [? [? [[? ?] ?]]].
   unfold func in *. rewrite H8,H5 in *. inv H. inv H0.
...
Inductive initial_node_state(fd: ident*func): env -> Prop := 
  | initial_state_node: forall e e1 fi,
      ...
      find_funct (node_block p) (acg_reset_name p.(node_main)) = Some fi -> 
