...
Lemma classify_fun_match:
  forall ty tyl,
  Clop.classify_fun ty = Clop.fun_case_f tyl Cltypes.Tvoid ->
  classify_fun (trans_type ty) = fun_case_f (trans_typelist tyl) Tvoid cc_default.
Proof.
  clear.
...
Proof.
 ...
 +change le2 with (set_opttemp None Vundef le2) at 2.
  apply exec_Scall with (trans_typelist (Ctemp.typelist_app tyargs tyrets)) Tvoid cc_default vf (vargs++vrets) (trans_fundef f); auto.
  -rewrite trans_expr_typeof; auto.
