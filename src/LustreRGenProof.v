...
Lemma eval_sassign:
  forall gc nk te te1 e e1 lh a cks,
  LsemS.eval_stmt progS gc nk te e te1 e1 (LustreS.Sassign lh (Expr a)) cks ->
  ~ In (fst lh) (get_lids a) ->
  eval_stmt true progR gc nk te e te1 e1 (Sassign (lvarof lh) a).
Proof.
  intros. inv H. inv H3. destruct e1 as [eh se].
...
Lemma trans_finit_correct:
  forall h s gc nk te e te1 e1,
  eval_stmts progS gc nk te e te1 e1 (initstmtsof2 h) ->
  trans_finit h = OK s ->
  eval_stmt true progR gc nk te e te1 e1 s.
Proof.
  intros.
  destruct h; simpl in *; try (inv H; inv H0; constructor; fail);
  unfold check_finit in *.
  +destruct (in_list _ _) eqn:?; inv H0.
   apply in_list_false_notin in Heqb0.
   inv H. inv H10. apply eval_sassign in H9; auto. inv H7.
  +destruct (in_list _ _) eqn:?; inv H0.
   apply in_list_false_notin in Heqb.
   inv H. inv H10. eapply eval_sassign; eauto. inv H7.
  +destruct (in_list _ _) eqn:?; inv H0.
   apply in_list_false_notin in Heqb.
   inv H. inv H10. eapply eval_sassign; eauto. inv H7.
  +inv H0. inv H. inv H10. eapply eval_sassign; eauto. inv H7.
  +destruct (in_list _ _) eqn:?; inv H0.
   apply in_list_false_notin in Heqb.
   inv H. inv H10. eapply eval_sassign; eauto. inv H7.
  +destruct (in_list _ _) eqn:?; inv H0.
...
Lemma trans_finit_fbyn_counter:
...
Lemma trans_hstmt_fbyn_counter:
  forall h s, trans_hstmt h = OK s ->
  fbyn_counter s = O.
Proof.
  destruct h; simpl; intros; try (monadInv H; auto).
...
Lemma trans_stmts_eqt_counter:
  forall l s,
  trans_stmts l = OK s ->
  eqt_counter (fbyeqsof l) = fbyn_counter s.
Proof.
  induction l; simpl; intros.
  inv H; auto.
  destruct a. monadInv H;simpl.
  rewrite <-IHl with x0; auto.
  destruct s0; simpl in *; try monadInv EQ; auto;
  rewrite check_clock_fbyn_counter; auto.
  +destruct (in_list _ _); try congruence.
   destruct e; simpl in *; try monadInv EQ; auto.
   destruct l0; monadInv EQ; auto.
   destruct l0; try congruence.
   destruct l0; inv EQ; auto.
   destruct (_ || _); inv EQ. destruct b; auto.
  +destruct (check_call _ _); try congruence. monadInv EQ. auto.
  +destruct b; monadInv EQ; simpl.
   -erewrite trans_finit_fbyn_counter; eauto.
    erewrite trans_hstmt_fbyn_counter; eauto.
   -erewrite trans_hstmt_fbyn_counter; eauto.
  +destruct (in_list _ _); try congruence. monadInv EQ.
   replace (fbyn_counter (cons_arydif p x1 l0 i)) with 0%nat; auto.
   clear. revert i. induction l0; simpl; auto.
  +destruct (in_list _ _); try congruence. monadInv EQ. auto.
  +rewrite IHl with x0; auto.
   replace (fbyn_counter x) with 0%nat; auto.
   destruct (in_list _ _); inv EQ.
   revert H0. clear. revert f l0 x.
   induction f; destruct l0; simpl; intros; monadInv H0; simpl in *; eauto.
  +destruct p. destruct (_ || _); inv EQ. auto.
  +destruct p0. destruct p0. destruct (in_list _ _); inv EQ.
   simpl. omega.
  +destruct (in_list _ _); try congruence. monadInv EQ. auto.
...
Lemma trans_stmts_lidsof_incl:
  forall l s,
  trans_stmts l = OK s ->
  incl (flat_map lidsof_eqt (fbyeqsof l)) (map fst (fbyvarsof s)).
Proof.
  induction l; simpl; intros.
  red; intros. tauto.
  destruct a. monadInv H. simpl. red; intros.
  rewrite map_app. rewrite flat_map_app in H.
  apply in_app_or in H.
  destruct H; apply in_or_app;[left | right].
  +destruct s0; simpl in *; try monadInv EQ; try tauto;
   rewrite check_clock_fbyvarsof.
   destruct H; try tauto. 
   subst. destruct p. destruct (_ || _); inv EQ. simpl. auto.
   destruct p0. destruct p0. destruct (in_list _ _); inv EQ.
   simpl in *. destruct H; auto.
   destruct H; try tauto. subst. simpl; auto.

   destruct (_ || _); inv EQ. simpl. auto. 
  +apply IHl; auto.
Qed.

Lemma lidsof_eqt_clockresetsof_incl:
  forall l, incl (flat_map lidsof_eqt (clockresetsof l)) (map fst (map fst l)).
Proof.
...
Lemma eval_clock_conds_true_check_clock:
  forall gc nid cks te te' e e' s,
  eval_clock_conds gc te (clock_conds cks) true ->
  eval_stmt true progR gc nid te e te' e' s ->
  eval_stmt true progR gc nid te e te' e' (check_clock cks s).
Proof.
  induction cks; simpl; intros; inv H; auto.
  destruct a. apply eval_Sif with v true; auto.
  destruct b; auto.
Qed.

Lemma eval_clock_conds_false_check_clock:
  forall gc nid cks te e s,
  eval_clock_conds gc te (clock_conds cks) false ->
  eval_stmt true progR gc nid te e te e (check_clock cks s).
Proof.
  induction cks; simpl; intros; inv H.
  destruct a. apply eval_Sif with v false; auto.
  destruct b; auto. constructor.
  destruct a. apply eval_Sif with v true; auto.
  destruct b; auto.
Qed.

Lemma check_call_list_norepeat_disjoint:
  forall l args,
  check_call l args = true ->
  list_norepet (ACG_I :: l) /\
    list_disjoint l (flat_map get_lids (arystr_sexps args)).
Proof.
  unfold check_call. intros.
  destruct (check_norepeat _) eqn:?; simpl in H; try congruence.
  destruct (list_in_list _ _) eqn:?; simpl in H; try congruence.
  apply check_norepeat_list_norepet in Heqb.
  apply list_in_list_disjoint in Heqb0.
  split; auto.
Qed.
...
Proof.
  ...
  ( P1 := fun nk te e te' e' s cks =>
      forall s', trans_stmt s (flagid cks) = OK s' ->
      eval_stmt true progR gc nk te e te' e' s')
...
+(*eval_node*)
   ...
   -unfold allvarsof in *; simpl in *.
    ...
    unfold eqtsof. rewrite eqt_counter_app.
    erewrite trans_stmts_eqt_counter; eauto.
    rewrite eqt_counter_clockresetsof; auto.
   -simpl. unfold eqtsof. rewrite flat_map_app. 
    red; intros. apply in_app_or in H7.
    rewrite map_app. apply in_or_app. 
    destruct H7; [right | left].
    eapply trans_stmts_lidsof_incl; eauto.
    apply lidsof_eqt_clockresetsof_incl; auto.
   -simpl. red; auto.
   -apply find_funct_in2 in H6. 
    apply ID_RANGE in H6; auto.
+(*eval_stmts_nil*)
...
+(*eval_stmts_cons_true*)
   monadInv H2. eapply eval_Sseq; eauto.
   eapply eval_clock_conds_true_check_clock; eauto.
   unfold clock_conds. rewrite map_rev; auto.
+(*eval_stmts_cons_false*)
   monadInv H1. eapply eval_Sseq; eauto.
   eapply eval_clock_conds_false_check_clock; eauto.
   unfold clock_conds. rewrite map_rev; auto.
+(*eval_Sassign*)
   simpl in *. destruct (in_list _ _) eqn:?; try congruence.
   eapply trans_expr_correct; eauto.
   apply in_list_false_notin; auto.
+(*eval_Scall*)
   simpl in H2. destruct (check_call _ _) eqn:?; monadInv H2.
   destruct IHeval_node as [fdR [vargs' [ef [ef' [A [A1 [A2 [A3 [A4 [A5 [A6 A7]]]]]]]]]]]; auto.
   destruct check_call_list_norepeat_disjoint with (map fst lhs) args as [A8 A9]; auto.
   generalize EQ; intros B.
   eapply trans_calldef_eq in B; eauto. subst.
   generalize H1; intros B1. inv A8.
...
+(*eval_Sarydif_cons*)
   simpl in *. destruct (in_list _ (_++_)) eqn:?; try congruence.
   rewrite in_list_app in Heqb. 
   apply orb_false_iff in Heqb. destruct Heqb. rewrite H6 in *.
   monadInv H4. apply eval_Sseq with te1 e; auto.
   constructor. constructor 1 with v v'; auto.
   unfold lvarof. simpl. 
   inv H2. eapply eval_sexp_lvalue_assign_disjoint; eauto.
   inv H0. inv H9. apply in_list_false_notin; auto.
+(*eval_Smix *)
   simpl in *. destruct (in_list _ (_++_)) eqn:?; try congruence.
   rewrite in_list_app in Heqb. apply orb_false_iff in Heqb.
   destruct Heqb. rewrite H5 in *.
   apply in_list_false_notin in H6; auto.
   monadInv H4. 
   apply eval_Sseq with te1 e; auto.
   inv H1. inv H16. inv H1.
   apply not_in_app in H6. destruct H6.
   assert(A: eval_lindexs gc te1 (typeof a3) ty lis Int.zero o).
     inv H4. eapply eval_lindexs_disjoint; eauto.
   apply eval_Sif with Vtrue true; simpl; auto.
   ...
   -apply cons_mixs_correct with lis (Svar b ty) Int.zero o v3 v b; eauto.
    inv H4. eapply eval_sexp_setnew; eauto.
    inv H4. constructor 1 with m'; auto.
    rewrite PTree.gss; auto. simpl. congruence.
+(*eval_Sstruct_nil*)
   monadInv H. constructor.
+(*eval_Sstruct_cons*)
   simpl in *. destruct (in_list _ (_++_)) eqn:?; inv H4.
   monadInv H6. rewrite in_list_app in Heqb.
   apply orb_false_iff in Heqb. destruct Heqb. rewrite H4 in *.
   apply eval_Sseq with te1 e; auto.
   constructor. constructor 1 with v v'; auto.
   unfold lvarof. simpl. 
   inv H1. eapply eval_sexp_lvalue_assign_disjoint; eauto.
   inv H5. inv H9. apply in_list_false_notin; auto.
+(*eval_Sskip*)
   inv H. constructor.
+(*fby_0*)
   simpl in *. destruct (_ || _) eqn:?; inv H4.
   apply orb_false_iff in Heqb. destruct Heqb.
   eapply eval_Sfby_cycle_1; eauto.
   econstructor; eauto.
   inv H3. apply eval_sexp_lvalue_assign_disjoint with v2 b ofs; eauto.
    inv H6. apply in_list_false_notin; auto.
+(*fby_n*)
   simpl in *. destruct (_ || _) eqn:?; inv H4.
   apply orb_false_iff in Heqb. destruct Heqb.
   eapply eval_Sfby_cycle_n; eauto.
   destruct lh. simpl. apply Pos.eqb_neq in H4.
   red; intros. apply H4. destruct H6; auto. tauto.
+(*fbyn_0*)
   simpl in *. destruct (in_list _ _) eqn:?; inv H10.
   eapply eval_Sfbyn_cycle_1; eauto.
   apply in_list_false_notin; auto.
+(*fbyn_n*)
   simpl in *. destruct (in_list _ _) eqn:?; inv H7.
   eapply eval_Sfbyn_cycle_n; eauto.
   rewrite in_list_app in Heqb. apply orb_false_iff in Heqb. destruct Heqb.
   apply in_list_false_notin. destruct lh; auto.
+(*arrow*)
   simpl in H2. destruct (in_list _ (_ ++ _)) eqn:?; inv H2.
   rewrite in_list_app in Heqb0. apply orb_false_iff in Heqb0.
   destruct Heqb0. eapply eval_Sarrow; eauto.
   assert(trans_stmt (LustreS.Sassign lh (Expr (if b then a1 else a2))) (flagid cks) =
            OK (Sassign (lvarof lh) (if b then a1 else a2))).
     destruct b; simpl. rewrite H2; auto. rewrite H3; auto.
   apply IHeval_node in H4. 
   destruct b; inv H4; auto.
+(*current_true*)
   monadInv H5.
+(*current_false*)
   monadInv H2.
...
+(*pre_cycle_n*)
   monadInv H4.
+(*currentR_true*)
   simpl in *. destruct (_ || _) eqn:?; try congruence.
   apply orb_false_iff in Heqb. destruct Heqb.
   rewrite in_list_app in H7. apply orb_false_iff in H7. destruct H7.
   monadInv H5. apply eval_Sif with Vtrue true; auto.
   rewrite H4; auto. 
   econstructor; eauto. econstructor; eauto.
   inv H3. apply eval_sexp_lvalue_assign_disjoint with v1 b ofs; eauto.
    inv H5. apply in_list_false_notin; auto.
+(*currentR_false*)
   simpl in *. destruct (_ || in_list _ (_ ++ _)) eqn:?; try congruence.
   apply orb_false_iff in Heqb. destruct Heqb.
   rewrite in_list_app in H4. apply orb_false_iff in H4. destruct H4.
   rewrite H3, H5 in *. simpl in *.
   monadInv H2. apply eval_Sif with Vfalse false; auto.
   rewrite H0; auto. 
+(*eval_Hmaptycmp*)
   monadInv H2. 
...
+(*eval_Hmapunary*)
   simpl in *. destruct (in_list _ _) eqn:?; try congruence.
   monadInv H4. rewrite H in *. unfold lvarof. simpl in *. inv EQ.
   ...
   destruct op; simpl; auto.
   inv H4. inv H8. apply in_list_false_notin; auto.
+(*eval_Hfoldary*)
...
+(*eval_Hfoldcast*)
   monadInv H0. econstructor; eauto. 
+(*eval_Farydef*)
   simpl in *. destruct (in_list _ _) eqn:?; monadInv H3.
   unfold lvarof. simpl in *.
   apply eval_Sassign; econstructor; eauto.
   inv H2. eapply eval_sexp_lvalue_assign_disjoint; eauto.
   inv H0. inv H6. apply in_list_false_notin; auto.
+(*eval_Haryslc*)
   simpl in *. destruct (in_list _ _) eqn:?; monadInv H2.
   apply eval_Sassign; econstructor; eauto.
   inv H1. eapply eval_sexp_lvalue_assign_disjoint; eauto.
   inv H2. inv H6. simpl. apply in_list_false_notin; auto.
+(*eval_Hboolred_true*)
...
+(*eval_Hboolred_false*)
   ...
   constructor.
+(*eval_Hmapcall*)
   simpl in H4. destruct (check_call _ _) eqn:?; monadInv H4.
   destruct IHeval_node as [fdR [vargs' [ef [ef' [A [A1 [A2 [A3 [A4 [A5 [A6 A7]]]]]]]]]]]; auto.
   destruct check_call_list_norepeat_disjoint with (map fst lhs) args as [A8 A9]; auto.
   generalize EQ0; intros B.
   eapply trans_hcalldef_eq in B; eauto. subst.
   generalize H3; intros B. inv A8.
   ...
   -eapply lids_disjoint_assign_lidst_disjoint; eauto.
    red; simpl; intros. eapply A9; eauto.
    erewrite <-trans_var_parys_get_lids; eauto.
    eapply trans_expr_projs_get_lids; eauto.
+(*eval_Hfillcall*)
   simpl in H5. destruct (check_call _ _) eqn:?; simpl in H5; try congruence.
   destruct (identeq _ _) eqn:?; simpl in H5; monadInv H5.
   destruct IHeval_node0 as [fdR [vargs' [ef [ef' [A [A1 [A2 [A3 [A4 [A5 [A6 A7]]]]]]]]]]]; auto.
   destruct check_call_list_norepeat_disjoint with (lid::map fst lhs) (Svar tid ty::nil) as [A8 A9]; auto.
   generalize EQ1; intros B.
   eapply trans_hcalldef_eq in B; eauto. subst.
   generalize H4; intros B. inv A8. inv H8.
   simpl in *. rewrite Heqb0 in *. simpl in *.
   eapply locenv_setarys_getmvls_mapw with (id:=lid) (ty:=ty) (v:=vret) (eh:=te1) in B; eauto. 
   destruct B as [vl1 B].
   apply eval_Sseq with te1 {| le := eh; sube := se |}; eauto.
   apply eval_Scall with ef ef' fdR vl1 (v::nil) vargs' (vret::vrs) i; simpl; auto.
   -eapply trans_hcalldef_load_loopid; eauto.
   -constructor 2; auto.
   ...
   -eapply lids_disjoint_assign_lidst_disjoint; eauto.
    econstructor; eauto.
    red; simpl; intros. eapply A9; simpl; eauto.
    destruct H5; auto. right. erewrite <-trans_var_parys_get_lids; eauto.
   -constructor; auto.
+(*eval_Hredcall*)
   simpl in H6. destruct (check_call _ _) eqn:?; simpl in H6; try congruence.
   destruct (identeq _ _) eqn:?; simpl in H6; monadInv H6. 
   destruct IHeval_node0 as [fdR [vargs' [ef [ef' [A [A1 [A2 [A3 [A4 [A5 [A6 A7]]]]]]]]]]]; auto.
   destruct check_call_list_norepeat_disjoint with (lid::nil) (Svar tid ty::args) as [A8 A9]; auto.
   generalize EQ1; intros B.
   eapply trans_hcalldef_eq in B; eauto. subst.
   generalize H5; intros B. inv A8. inv H9. 
   simpl in *. rewrite Heqb0 in *. simpl in *.
   econstructor; eauto. eapply eval_map_args in H1; eauto.
   assert(B1: exists vl, locenv_getmvls gc te1 (Svar lid ty :: nil) vl).
     apply locenv_setarys_getmvls_mapw with vret te2 nil nil nil te2; auto.
     constructor. constructor; auto.
   destruct B1 as [vl1 B1].
   apply eval_Scall with ef ef' fdR vl1 (v::vargs) vargs' (vret::nil) i; simpl; auto.
   -eapply trans_hcalldef_load_loopid; eauto.
   -constructor 2; auto.
   -erewrite trans_expr_proj_typeof; eauto.
   -constructor 2 with te2; auto. constructor.
   -constructor 2; simpl; auto.
   -erewrite trans_expr_proj_typeof; eauto.
   -eapply lids_norepet_lvalues_norepet; eauto.
   ...
   -eapply lids_disjoint_assign_lidst_disjoint; eauto.
    econstructor; eauto.
    red; simpl; intros. eapply A9; simpl; eauto.
    destruct H6; try tauto.
    destruct (is_arystr ty); simpl in *.
    destruct H7; auto. right. eapply trans_expr_projs_get_lids; eauto.
    eapply trans_expr_projs_get_lids; eauto.
+(*eval_Hmapfoldcall*)
   simpl in H7. destruct (check_call _ _) eqn:?; simpl in H7; try congruence.
   destruct (identeq _ _) eqn:?; simpl in H7; monadInv H7. 
   destruct IHeval_node0 as [fdR [vargs' [ef [ef' [A [A1 [A2 [A3 [A4 [A5 [A6 A7]]]]]]]]]]]; auto.
   destruct check_call_list_norepeat_disjoint with (lid::map fst lhs) (Svar tid ty::args) as [A8 A9]; auto.
   generalize EQ0; intros B.
   eapply trans_hcalldef_eq in B; eauto. subst.
   generalize H6; intros B. inv A8. inv H10.
   simpl in *. rewrite Heqb0 in *. simpl in *.
   eapply locenv_setarys_getmvls_mapw with (id:=lid) (ty:=ty) (v:=vret) (eh:=te1) in B; eauto. 
...
Proof.
 ...
 +(*init_node*)
  ...
  -rewrite trans_stmts_fbyvarsof with (l:=nd_stmt f); auto.
  -erewrite trans_stmts_instidof_eq; eauto.
  -apply find_funct_in2 in H2. 
   apply ID_RANGE in H2; auto.
 +(*nil*)
  constructor.
 +(*cons*)
