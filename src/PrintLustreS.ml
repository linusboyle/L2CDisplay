...
let print_hstmt p s =
  ...
  | Hfillcall((id,ty), _, lh, c, e1) ->
      fprintf p "@[<hv 2>fillcall: %s,%s =@ %s.%s@,(@[<hov 0>%a@]);@]"
                (extern_atom id) 
                (print_lhs lh)
                (extern_atom c.callid)
                (extern_atom c.instid)
                print_sexp e1
  | Hredcall((id,ty), _, c, e1, el) ->
      fprintf p "@[<hv 2>redcall: %s =@ %s.%s@,(@[<hov 0>%a, %a@]);@]"
                (extern_atom id) 
                (extern_atom c.callid)
                (extern_atom c.instid)
                print_sexp e1
...
let rec print_stmt p s =
  ...
  | Sassign((id,ty), Ecase(e, cases)) ->
      fprintf p "@[<v 2>%s =@ switch (%a) {@ %a@;<0 -2>}@]"
              ...
              print_sexp e
              print_cases cases
  | Sassign((id,ty), Emerge(e, cases)) ->
      fprintf p "@[<v 2>%s =@ merge (%a) {@ %a@;<0 -2>}@]"
              (extern_atom id)
              print_sexp e
              print_cases cases
  ...
  | Sfbyn ((id,ty),(id1,id2),n, e1, e2) ->
      fprintf p "@[<hv 2>%s =@ fby@,(@[<hov 0>%a,%ld,%a@]);@]"
                ...
                (camlint_of_coqint n)
                print_sexp e2
  | Scurrent((id,ty), _, e, e1) ->
      fprintf p "@[<hv 2>%s =@ current@,(@[<hov 0>%a,%a@]);@]"
                (extern_atom id)
                print_sexp e
                print_sexp e1
  | ScurrentR((id,ty), _, e, e1, e2) ->
      fprintf p "@[<hv 2>%s =@ current@,(@[<hov 0>%a,%a,%a@]);@]"
                (extern_atom id)
                print_sexp e
                print_sexp e1
                print_sexp e2
  | Spre ((id,ty),id1,e1) ->
      fprintf p "@[<hv 2>%s =@ fby@,(@[<hov 0>%a@]);@]"
                (extern_atom id)
                print_sexp e1
  | Scall(lhs, e1, el) ->
...
let rec str_clock cks=
  match cks with
   [] -> ""
  | (ck, cid) :: tl -> 
    (if ck then (extern_atom cid) else "NOT " ^ (extern_atom cid)) 
      ^ (if tl = [] then "" else " -> ") ^ (str_clock tl)

let print_stmtclock p (s, cks) =
  print_stmt p s;
  fprintf p " (%s)" (str_clock cks)
...
let print_node p (id, f) =
  ...
  List.iter
    ...
    f.nd_vars;
  List.iter (fun a -> print_stmtclock p a) f.nd_stmt;
  fprintf p "@;<0 -2>}@]@ @ "
