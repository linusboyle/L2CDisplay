...
Lemma eval_expr_match:
  forall gc e1 s t v,
  eval_expr gc e1 s t v ->
  forall l e2 ,ptree_noids_match (map fst (mkloopmapw l)) e1 e2 -> 
  ptree_ids_none (map fst (mkloopmapw l)) e1 -> 
  env_none gc ->
  eval_expr gc e2 s t v.
Proof.
...
Lemma eval_sassign_correct:
  forall gc k l e e1 te1 te1' ta ta1 ta2 te2 p s cks,
  LsemT.eval_stmt false progT gc k te1 e te1' e1 (Sassign p s) cks ta ta1 ->
  locenv_matcht (mkloopmapw l) ta2 te1 te2 ->
  env_none gc ->
  exists te2', eval_stmt progT gc k te2 e te2' e1 (Sassign p s) cks
    /\ locenv_matcht (mkloopmapw l) ta2 te1' te2' .
Proof.
  intros. inv H. 
...
Proof.
  ...
  +inv H0. inv H12. eapply eval_sassign_correct in H11; eauto.
   destruct H11 as [e2' [? ?]]. exists e2'; split; eauto.
   repeat (econstructor; eauto). inv H9.
  +inv H0. inv H12. eapply eval_sassign_correct in H11; eauto.
   destruct H11 as [e2' [? ?]]. exists e2'; split; eauto.
   repeat (econstructor; eauto). inv H9.
  +inv H0. inv H12. eapply eval_sassign_correct in H11; eauto.
...
Lemma eval_clock_conds_match:
  forall gc te1 cks b,
  eval_clock_conds gc te1 cks b ->
  forall te2,
  locenv_match te1 te2 /\ locenv_disjoint gc te1 te2 ->
  eval_clock_conds gc te2 cks b.
Proof.
  induction 1; simpl; intros.
  constructor.
  constructor 2 with v; auto.
  destruct H2. eapply eval_sexp_match; eauto.
  constructor 3 with v; auto.
  destruct H3. eapply eval_sexp_match; eauto.
Qed.
...
Proof.
  ...
  ( P0 := fun a1 a2 num aty i b =>
      ...
      (forall id o k, eval_lvalue gc eh a1 id o k -> eval_lvalue gc eh a3 id o k) ->
      (forall id o k, eval_lvalue gc eh a2 id o k -> eval_lvalue gc eh a4 id o k) ->
      eval_arycmp gc eh a3 a4 num aty i b)
  ( P1 := fun a1 a2 fld ftl b =>
      forall a3 a4, typeof a1 = typeof a3 ->
      typeof a2 = typeof a4 ->
      (forall v, eval_sexp gc eh a1 v -> eval_sexp gc eh a3 v) ->
      ...
      (forall id o k, eval_lvalue gc eh a1 id o k -> eval_lvalue gc eh a3 id o k) ->
      (forall id o k, eval_lvalue gc eh a2 id o k -> eval_lvalue gc eh a4 id o k) ->
      eval_strcmp gc eh a3 a4 fld ftl b);
  intros.
 ...
 +apply eval_typecmp_array with aid aty num v1 v2; auto. 
  congruence. congruence.
 +apply eval_typecmp_struct with sid fld v1 v2; auto.
  congruence. congruence.
...
Proof.
  ...
  ( P0 := fun nk te1 e te1' e' ss =>
      ...
      exists te2', eval_stmts progT gc nk te2 e te2' e' ss
       /\ locenv_matcht (mkloopmapw l) empty_locenv te1' te2')
  ( P1 := fun nk te1 e te1' e' s cks ta ta' =>
      forall l te2, 
      locenv_matcht (mkloopmapw l) ta te1 te2 ->
      env_none gc ->
      has_for s <= has_fors l ->
      exists te2', eval_stmt progT gc nk te2 e te2' e' s cks
        /\ locenv_matcht (mkloopmapw l) ta' te1' te2')
  ( P2 := fun nk te1 e te1' e' f ta =>
      forall l te2, 
      locenv_matcht (mkloopmapw l) ta te1 te2 ->
      env_none gc ->
      exists te2', eval_hstmt progT gc nk te2 e te2' e' f
        /\ locenv_matcht (mkloopmapw l) ta te1' te2')
  ( P3 := fun nk te1 se se1 atys vargs i cdef rtys vrets =>
      forall l1 ta te2, 
      locenv_matcht (mkloopmapw l1) ta te1 te2 ->
      env_none gc ->
      eval_apply progT gc nk te2 se se1 atys vargs i cdef rtys vrets); simpl; intros.
  +(*eval_node*)
   subst.
  ...
  +(*eval_stmts_cons_false*)
   destruct IHeval_node with l te2 as [te2' [? ?]]; auto.
     destruct (zlt _ _). 
     apply Zle_trans with (Z.max (has_for s) (has_fors ss)); xomega.
     generalize (has_fors_range ss). intros. omega.
   exists te2'; split; auto. econstructor 3; eauto.
   eapply eval_clock_conds_match; eauto.
   eapply locenv_matcht_simpl; eauto.
  +(*eval_Sassign*)
  ...
  +(*eval_Haryslc*)
    ...
    *inv H0. apply eval_Rlvalue with id ofs k; auto.
     inv H10. econstructor; eauto. eapply eval_lvalue_match; eauto.
     inv H16. apply eval_Sbinop with v1 v2; auto.
     eapply eval_sexp_const; eauto.
     inv H19; auto. inv H0. inv H0. simpl in *.
  ...
  +(*eval_Hfillcall*)
   ...
   generalize H3 H4; intros C C1.
   eapply locenv_matcht_setlvar_exists in H3; eauto.
   destruct H3 as [te22 [? ?]]. 
   eapply locenv_matcht_setarys_exists in H4; eauto.
   destruct H4 as [te2' [? ?]].
   exists te2'. split; auto.
   -apply eval_Hfillcall with te21 te22 i v vret vrs tys; eauto.
    *eapply eval_sexp_match; eauto.
    *destruct A1 as [? [? [? [? ?]]]].
     eapply eval_sexp_svar_match; eauto.
     destruct H6; auto.
  +(*eval_Hredcall*)
   destruct IHeval_node with l te0 as [te21 [A A1]]; simpl; auto.
     generalize (has_fors_range l). omega.
   destruct locenv_matcht_simpl with gc l ta te1 te21 as [A2 A3]; auto.
   generalize H5; intros C.
   eapply locenv_matcht_setlvar_exists in H5; eauto.
   destruct H5 as [te2' [? ?]]. 
   exists te2'. split; auto.
   -apply eval_Hredcall with te21 i v atys vl vargs vret; eauto.
    *eapply eval_sexp_match; eauto.
    *eapply eval_sexps_match; eauto.
    *destruct A1 as [? [? [? [? ?]]]].
     eapply eval_sexp_svar_match; eauto.
  ...
  +(*eval_apply*)
   ...
   destruct H7. eapply nodes_id_ptree_none; eauto.
Qed.
