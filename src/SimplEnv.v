...
Fixpoint check_svar_types(tys: list type): bool :=
  match tys with
  | nil => true
  | ty :: tyl => zlt 0 (sizeof ty) && check_svar_types tyl
  end.
...
Definition trans_body(b: func): func :=
  ...
  (mknode b.(nd_kind) (nd_args b) ((OUTSTRUCT, sty)::nil) nil nil (nd_vars b)
  ...
  if nd_kind (snd fd) then 
    ...
    | _ =>
      if zlt 0 (sizeof_fld (nd_fld (snd fd))) && zle (sizeof_fld (nd_fld (snd fd))) Int.max_signed then
        if check_svar_types (map snd (svarsof (snd fd))) then 
          OK (fst fd, trans_body (snd fd))
        else Error (msg "sizeof svars is zero!")
...
Proof.
  ...
  destruct (check_svar_types _); inv H; auto. 
Qed.