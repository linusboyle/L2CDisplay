...
Proof.
  unfold call_node. intros.
  destruct H as [? [? [[? [? [? [? [? ?]]]]] [? [? ?]]]]].
  generalize H18 H18; intros A A1.
  ...
  +rewrite <-H4, <-A3 in *; simpl in C.
   ...
   -simpl typeof. exists m, (mknstruct (snd nd)). 
    ...
    apply alignof_1248.
    erewrite field_offset_unsigned_repr; eauto.
    eapply field_offset_aligned in H18; eauto.
    omega.
    ...
    *eapply subenv_struct_match_in in H27; eauto.
     rewrite Int.add_zero_l in H27. simpl in H23, H24.
     assert(B: env_list_match m (Int.repr ofs) fd (nat_of_int j) efs).
       clear -H27. inv H27. apply int_unsigned_inj in H3. congruence.
  ...
  +rewrite <-H7 in *. simpl in C.
    ...
    *eapply subenv_struct_match_in in H27; eauto.
     rewrite Int.add_zero_l in H27.
     unfold nat_of_int in H22. rewrite Int.unsigned_zero in *.
     simpl intof_opti in *. rewrite Int.unsigned_one in *.
     simpl in H22.
     assert(B: efs = mkenv ehf sef :: nil).
       destruct efs; inv H22. simpl length in *. rewrite Nat2Z.inj_succ in H24.
       destruct efs; simpl length in *; auto. rewrite Nat2Z.inj_succ in H24. omega.
     subst efs.
...
Proof.
  ...
  try destruct (_ && _) eqn:?;
  try  destruct (check_svar_types _);
...
Proof.
  unfold Lustre.call_func, call_func. intros.
  destruct H as [? [? [? ?]]].
  intuition.
...
Proof.
  unfold call_node, call_func. intros. rewrite H0 in *.
  destruct H as [? [? [[? [? [? [? ?]]]] ?]]].
  monadInv TRANSL. simpl in *.
  eapply find_funcs_monad_exits in H3; eauto. destruct H3 as [fd' [? ?]].
  exists fd'. unfold trans_node in *.
  unfold func in *. rewrite <-H4, H0 in *.
  destruct (nd_fld (snd fd)) eqn:?; try congruence.
  destruct (_ && _); try congruence.
  destruct (check_svar_types _); inv H3.
  intuition.
  ...
  +unfold trans_node. intros. unfold func in *.
   ...
   destruct (_ && _); try congruence. 
   destruct (check_svar_types _); inv H9. auto.
Qed.
...
Proof.
  ...
  +inv H13. inv H14. destruct H1 as [? [? [[? [? [? [? ?]]]] ?]]].
   ...
   unfold func in *. rewrite H16, <-H17, H in *.
   simpl in *. congruence.
...
Proof.
  ...
  +inv H13. inv H14. destruct H1 as [? [? [[? [? [? [? ?]]]] ?]]].
   rewrite H15 in *. inv H1.
   rewrite H, H17 in *. simpl in *. destruct H22. congruence.
  +inv H13. inv H14. destruct ef' as [ef' sef].
   apply eval_Scall with ef ef' vl fd vargs vargs' vrets; auto.
   eapply trans_call_func; eauto.
   destruct H0 as [? [? [? [? [? ?]]]]].
...
Proof.
 ...
 +(*node*)
  ...
  destruct (zlt _ _), (zle _ _); try destruct (check_svar_types _) eqn:?; inv H9. 
 ...
 +(*Scall_node*)
  ...
  destruct (nd_fld (snd fd)) eqn:?; rewrite A10 in *.
  ...
  assert(A14: callid cdef = fst fd).
    eapply find_funct_fid in A3; eauto. 
  generalize A8; intros A15.
  ...
  -clear -B5 B6 B7 B8 A11 H Heqf. 
   ...
   destruct (_ && _); try destruct (check_svar_types _); inv A11.
  ...
  -unfold trans_node in A11. unfold func in *. 
   rewrite H, Heqf in *. destruct (_ && _); try destruct (check_svar_types _); inv A11.
   simpl. congruence.
  -rewrite trans_exprs_typeof; auto.
   unfold trans_node in A11. unfold func in *. rewrite H, Heqf in *.
   destruct (_ && _); try destruct (check_svar_types _); inv A11. auto.
...
Proof.
  ...
  rewrite H6 in *. simpl in *. destruct H9. congruence.
Qed.  
...
Proof.
  ...
  simpl in *. inv H2.
  eapply alloc_stmt_empty_env with (m:=m) (i:=i) in H5; eauto. 
  destruct H5. repeat (split; simpl; auto).
  +constructor.
  +constructor 2; auto.
  +red; intros; auto.
   rewrite PTree.gempty in *. congruence.
...
Lemma check_svar_types_sizeof:
  forall tys, check_svar_types tys = true ->
  (forall ty, In ty tys -> 0 < sizeof ty).
Proof.
  induction tys; simpl; intros.
  tauto.
  destruct (zlt _ _); destruct (check_svar_types _) eqn:?; simpl in H; try congruence.
...
Proof.
 ...
 +(*alloc_node*)
  unfold trans_node in *. rewrite H5, H6 in *. rewrite <-H6 in *.
  destruct (zlt _ _), (zle _ _); try destruct (check_svar_types _) eqn:?;inv H3.
   ...
   *constructor 1; auto. eapply alloc_variables_locals_struct_match; eauto.      
    ...
    apply check_svar_types_sizeof; auto.  
   -rewrite PTree.gss; auto.
   -constructor; auto. unfold func in *. rewrite H6; congruence.
 +(*nil*)
  ...
  econstructor 2 with (list_repeat (nat_of_int (intof_opti (callnum c))) empty_env) delta fd _; simpl; eauto.
   ...
   destruct (callnum c); constructor; simpl.
    apply env_list_match_alloc; auto; try omega.
      simpl in C2, A6. unfold nat_of_int. rewrite nat_of_Z_eq; try omega.
...
Proof.
  ...
  +generalize H1; intros C.
   ...
   -constructor 2 with e2'; auto.
    ...
    destruct (_ && _); try destruct (check_svar_types _); inv H5. auto.
...
Proof.
  ...
  +unfold trans_node in *.
   ...
   destruct (zlt _ _), (zle _ _); simpl in A3, A5; try congruence.
   destruct (check_svar_types _) in A3; inv A3.
   destruct (check_svar_types _) in A5; inv A5.
  ...
  +unfold trans_node. intros.
   destruct (nd_kind (snd x0)); try congruence.
   destruct (nd_fld (snd x0)); try congruence.
   destruct (_ &&_ _) in H; try destruct (check_svar_types _); inv H. auto.
  +unfold trans_node. intros.
   destruct (nd_kind (snd x0)); try congruence.
   destruct (nd_fld (snd x0)); try congruence.
   destruct (_ && _) in H; try destruct (check_svar_types _); inv H. auto. 
Qed.
