...
Definition type_of_function (f: function) : type :=
  Tfunction (type_of_params (fn_params f ++ fn_temps f)) (fn_return f).
...
Inductive exec_stmt: env -> temp_env -> temp_env -> mem -> statement -> trace -> mem -> outcome -> Prop :=
  ...
  | exec_Smemcpy:   forall e le te m ty tyargs al size algn bdst odst bsrc osrc t m' vres,
      ...
      exec_stmt e le te m (Smemcpy ty tyargs al)
                t m' Out_normal
  | exec_Scall:   forall e le te m a al rets tyargs tyrets vf vargs vrets f t m',
      classify_fun (typeof a) = fun_case_f (typelist_app tyargs tyrets) Tvoid ->
      eval_expr e le te m a vf ->
      eval_exprlist e le te m al tyargs vargs ->
      eval_exprlist e le te m rets tyrets vrets ->
      Genv.find_funct ge vf = Some f ->
      type_of_fundef f = Tfunction (typelist_app tyargs tyrets) Tvoid ->
...
Inductive type_of_fundef_case:type -> type -> type -> type -> bool -> Prop :=
  | type_of_fundef_anil_rnil: 
    type_of_fundef_case (Tfunction Tnil Tvoid) (Tfunction Tnil Tvoid) Tvoid Tvoid false
  | type_of_fundef_anil_rcons: forall oty,
    type_of_fundef_case (Tfunction (Tcons (Tpointer oty) Tnil) Tvoid) (Tfunction (Tcons (Tpointer oty) Tnil) Tvoid) Tvoid oty true
  | type_of_fundef_acons_rnil: forall ity,
    type_of_fundef_case (Tfunction (Tcons (Tpointer ity) Tnil) Tvoid) (Tfunction Tnil Tvoid) ity Tvoid false
  | type_of_fundef_acons_rcons: forall ity oty,
    type_of_fundef_case (Tfunction (Tcons (Tpointer ity) (Tcons (Tpointer oty) Tnil)) Tvoid) (Tfunction (Tcons (Tpointer oty) Tnil) Tvoid) ity oty true.
...
Inductive initial_state(f: fundef): mem -> Prop :=
  | initial_state_: forall b1 b2 fi m_init m0 m1 m2 ity oty b,
      ...
      Genv.find_symbol ge (acg_reset_name p.(prog_main)) = Some b1 ->
