...
Inductive unary_operationL : Type :=
  | Obool: unary_operationL              (**r convert to bool(signed Ibool bits) ([bool]) *)
  | Ochar: unary_operationL              (**r convert to char(signed I8 bits) ([char]) *)
  | Oshort : unary_operationL            (**r convert to short(signed 16 bits) ([short]) *)
  | Oint : unary_operationL              (**r convert to int(signed 32 bits) ([int]) *)
  | Ofloat : unary_operationL            (**r convert to float(32 bits) ([float]) *)
  | Oreal : unary_operationL             (**r convert to real(64 bits) ([real]) *)
  | Onot : unary_operationL              (**r boolean negation ([not]) *)
  | Opos : unary_operationL              (**r positive (unary [+]) *)
  | Oneg : unary_operationL.             (**r opposite (unary [-]) *)

Inductive binary_operationL : Type :=
  | Oadd : binary_operationL             (**r addition (binary [+]) *)
  | Osub : binary_operationL             (**r subtraction (binary [-]) *)
  | Omul : binary_operationL             (**r multiplication (binary [*]) *)
  | Odivreal : binary_operationL         (**r division real ([/]) *)
  | Odivint : binary_operationL          (**r division integer ([div]) *)
  | Omod : binary_operationL             (**r remainder ([mod]) *)
  | Oand : binary_operationL             (**r and ([and]) *)
  | Oor : binary_operationL              (**r or ([or]) *)
  | Oxor : binary_operationL             (**r xor ([xor]) *)
  | Oeq: binary_operationL               (**r comparison ([=]) *)
  | One: binary_operationL               (**r comparison ([<>]) *)
  | Olt: binary_operationL               (**r comparison ([<]) *)
  | Ogt: binary_operationL               (**r comparison ([>]) *)
  | Ole: binary_operationL               (**r comparison ([<=]) *)
  | Oge: binary_operationL.              (**r comparison ([>=]) *)
