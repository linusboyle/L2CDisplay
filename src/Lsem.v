...
Definition eval_const(c: const): val :=
  ...
  | Cenum id idl => Vint (Int.repr (count_occ_pos idl id 0))
  end.
...
Definition eval_patn(p: patn): option int :=
  ...
  | Paenum id idl => Some (Int.repr (count_occ_pos idl id 0))
  | Pany => None
  end.
...
Inductive eval_typecmp(te: locenv)(a1 a2: sexp) : bool -> Prop :=
  ...
  | eval_typecmp_array: forall aid aty num b v1 v2,
      ...
      eval_sexp te a1 v1 ->
      eval_sexp te a2 v2 ->
      eval_arycmp te a1 a2 num aty Int.zero b ->
      eval_typecmp te a1 a2 b
  | eval_typecmp_struct: forall sid fld b v1 v2,
      ...
      eval_sexp te a1 v1 ->
      eval_sexp te a2 v2 ->
      eval_strcmp te a1 a2 fld fld b ->
      eval_typecmp te a1 a2 b

with eval_arycmp(te: locenv)(a1 a2: sexp): Z -> type -> int ->  bool -> Prop :=
  | eval_arycmp_loop: forall num aty i b1 b2,
      Int.unsigned i < num <= Int.max_signed ->
      eval_typecmp te (Saryacc a1 (Sconst (Cint i) type_int32s) aty) (Saryacc a2 (Sconst (Cint i) type_int32s) aty) b1 ->
      eval_arycmp te a1 a2 num aty (Int.add i Int.one) b2 ->
      eval_arycmp te a1 a2 num aty i (b1 && b2)
  | eval_arycmp_false: forall aty i,
      eval_arycmp te a1 a2 (Int.unsigned i) aty i true

with eval_strcmp(te: locenv)(a1 a2: sexp): fieldlist -> fieldlist ->  bool -> Prop :=
  | eval_strcmp_cons: forall fld id ty ftl b1 b2,
      eval_typecmp te (Sfield a1 id ty) (Sfield a2 id ty) b1 ->
      field_type id fld = OK ty ->
      eval_strcmp te a1 a2 fld ftl b2 ->
      eval_strcmp te a1 a2 fld (Fcons id ty ftl) (b1 && b2)
  | eval_strcmp_nil: forall fld,
      eval_strcmp te a1 a2 fld Fnil true.
...
Inductive eval_clock_conds(gc te: locenv): list sexp -> bool -> Prop :=
  | eval_clock_conds_nil:
    eval_clock_conds gc te nil true 
  | eval_clock_conds_cons_false: forall cond conds v,
    eval_sexp gc te cond v ->
    bool_val v type_bool = Some false ->
    typeof cond = type_bool ->
    eval_clock_conds gc te (cond::conds) false
  | eval_clock_conds_cons_true: forall cond conds v b,
    eval_sexp gc te cond v ->
    bool_val v type_bool = Some true ->
    typeof cond = type_bool ->
    eval_clock_conds gc te conds b -> 
    eval_clock_conds gc te (cond::conds) b. 
...
Inductive eval_fbyeqs(gc te: locenv): locenv -> locenv -> list (eqt*list sexp) -> Prop :=
  ...
  | eval_fbyeqs_cons_true: forall e e1 e2 eq eql conds,
      ...
      eval_fbyeqs gc te e e2 ((eq, conds) :: eql)
  | eval_fbyeqs_cons_false: forall e e2 eq eql conds,
      eval_clock_conds gc te (rev conds) false ->
      eval_fbyeqs gc te e e2 eql ->
      eval_fbyeqs gc te e e2 ((eq, conds) :: eql).

Inductive eval_init: locenv -> list (ident*type) -> list (ident*type)-> locenv ->  Prop :=
  | eval_init_: forall flags svars eh eh1 eh2,
     alloc_variables eh (flags ++ svars) eh1 ->
     (forall ty, In ty (map snd flags) -> ty = type_bool) ->
     locenv_setvars eh1 flags (list_repeat (length flags) Vtrue) eh2 ->
     eval_init eh flags svars eh2.
...
Lemma eval_fbyeq_determ:
  forall gc te e e1 eq,
  eval_fbyeq gc te e e1 eq ->
  forall e2, eval_fbyeq gc te e e2 eq ->
  e1 = e2.
Proof.
  intros; inv H; inv H0. 
  +eapply eval_sexp_determ in H1; eauto. subst.
   eapply eval_cast_determ in H2; eauto. subst.
   apply locenv_setlvar_determ with (e1:=e2) in H3; auto.
  +eapply eval_eqf_determ in H1; eauto.
Qed.

Lemma eval_clock_conds_determ:
  forall gc te cks b1,
  eval_clock_conds gc te cks b1 ->
  forall b2, eval_clock_conds gc te cks b2 ->
  b1 = b2.
Proof.
  induction 1; simpl; intros.
  +inv H. auto.
...
Proof.
  ...
  +inv H; auto.
  +inv H2. 
   -eapply eval_fbyeq_determ in H0; eauto.
    subst; auto.
   -eapply eval_clock_conds_determ in H; eauto.
    congruence.
  +inv H1; auto.
   eapply eval_clock_conds_determ in H; eauto.
   congruence.
...
Lemma eval_typecmp_determ:
  forall gc eh a1 a2 b1,
  eval_typecmp gc eh a1 a2 b1 ->
  forall b2, eval_typecmp gc eh a1 a2 b2 ->
  b1 = b2.
Proof.
  intros until eh.
  induction 1 using eval_typecmp_ind2 with 
  ( P0 := fun a1 a2 num aty i b1 =>
      forall b2, eval_arycmp gc eh a1 a2 num aty i b2 ->
      b1 = b2)
  ( P1 := fun a1 a2 fld ftl b1 =>
      forall b2, eval_strcmp gc eh a1 a2 fld ftl b2 ->
      b1 = b2); intros.
 +inv H3. eapply eval_sexp_determ in H1; eauto. subst.
  congruence.
  rewrite H5 in *. simpl in *. congruence.
  rewrite H5 in *. simpl in *. congruence.
 +inv H4. rewrite H0 in *. simpl in *. congruence.
  rewrite H6 in H0. inv H0. auto.
...
Lemma locenv_setvars_notin_eq:
  forall eh l vl eh',
  locenv_setvars eh l vl eh' ->
  forall id, ~ (In id (map fst l)) ->
  eh' ! id = eh ! id.
Proof.
  induction 1; simpl; intros; auto.
  rewrite IHlocenv_setvars; auto.
